...
xlsx_file = pd.ExcelFile('data.xlsx').parse('Sheet1')
...
network = cg724.construct_RNN([1,2,2,1],delay_Input=[0],delay_Intern=[1],
                          delay_Output=[1,2])
...
network = cg724.lm_Algorithm(Inputs,Targets,network,verbose=True,k_max=100,
                             E_stop=1e-3)

targets = cg724.output_RNN_b(Inputs,network)
targets_Test = cg724.output_RNN_b(Inputs_Test,network)
