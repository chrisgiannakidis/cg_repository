...
def construct_RNN(nn,delay_Input=[0],delay_Intern=[],delay_Output=[]):
	...
	Args:
		...
		delay_Input: Time delays for NN inputs. 
			To use only the input of timestep t delay_Input = [0]
			delay_Input = [0,1,5] will use inputs of timestep t,t-1 and t-5
		delay_Intern: Time delays for recurrent internal connections of NN.
			delay_Intern has to be greater than zero (layer output at timestep t-x)!
			if a non empty list is given, recurrent connection from every
			layer to itself and every layer before is added.
			delay_Intern = [1,2,5] adds a recurrent connection for the output of
			each layer in timestep t-1,t-2 and t-5
		delay_Output: Time delays for recurrent connections of output to first hidden layer.
			delay_Output has to be greater than zero (output at timestep t-x)!
			if a non empty list is given, recurrent connection from NN output
			to first hidden layer is added.
			delay_Output = [1,3,4] adds a recurrent connection for the output of
	...
	net = {} #neural network
	delay = {'In':delay_Input,'Intern':delay_Intern,'Out':delay_Output} #Time delays
	net['delay'] = delay #Time delays	
	net['nn'] = nn #structure
	net['M'] = len(nn)-1 #number of layers of the NN
	net['layers'] = nn[1:] #structure without inputs
	net['dmax'] = max(max(delay_Input,delay_Intern,delay_Output)) # maximum time delay	
	net = construction(net) #initialize random weight vector and specify sets
	net['w'] = net['w0'].copy() #weight vector used for calculation
	net['N'] = len(net['w0']) #number of weights
	return net
	
def construction(net):
	"""	Constructs random weight vector of NN and defines sets needed for
	...
	net['CX_LW'] = CX_LW
	net['CU_LW'] = CU_LW
	net['w0'] = convert_Matrix_To_Vector(net,IW,LW,b)
	return net
	
def convert_Matrix_To_Vector(net,IW,LW,b):
...
def convert_Vector_To_Matrix(net):
...
def output_RNN_a(P,net,IW,LW,b,a={},q0=0):
...
def output_RNN_b(P,net,P0=None,Y0=None):
	"""	Calculates NN Output for given Inputs P
		User Function
	...
	data,net = formulate_Data(P,Y,net,P0=P0,Y0=Y0)
	IW,LW,b = convert_Vector_To_Matrix(net) #input-weight matrices,connection weight matrices, bias vectors
	Y_NN = output_RNN_a(data['P'],net,IW,LW,b,a=data['a'],q0=data['q0'])[0]
...
def rtrl_Training(net,data):
	...
	Y_NN,n,a = output_RNN_a(P,net,IW,LW,b,a=a,q0=q0)
	...
	#########################
	# 3. Backpropagation rtrl_Training
...
def bptt_Training(net,data):
	"""	Implementation of the Back Propagation Through Time Algorithm based on:
		...
		Proceedelay_Inputgs of the IEEE, Nummer 10, Vol. 78 (1990), S. 1550-1560.
	...
	for q in range(Q,q0,-1):
		...
		dE_dw = convert_Matrix_To_Vector(net,dE_dIW,dE_dLW,dE_db)#convert derivative matrices to derivative vector
...
def lm_Algorithm(P,Y,net,k_max=100,E_stop=1e-10,dampfac=3.0,dampconst=10.0,\
			verbose = False):
	...
	"""
	#create data dict
	data,net = formulate_Data(P,Y,net)
	
	#Calculate Jacobian, Error and error vector for first iteration
	J,E,e = rtrl_Training(net,data)
	k = 0
	ErrorHistory=np.zeros(k_max+1) #Vektor for Error hostory
	ErrorHistory[k]=E
	if verbose:
		print('Epoch: ',k,' Iterations','     Performance (error): ',E,
        '     scale factor: ',dampfac)
	...
	#run loop until either k_max or E_stop is reached
		...
		#repeat until optimizing step is successful
			...
			Enew = compute_Error(net,data) #calculate new Error E
...
def compute_Error(net,data):
	"""	Compute Error for NN based on data
...
def formulate_Data(P,Y,net,P0=None,Y0=None):
	"""	Formulate Input Data for the use for RNN Training and check for errors
...
def save_RNN(net,filename):
	"""	Save recurrent neural network object to file
	...
	#write input delays delay_Input
	writer.writerow(['delay_Input'])
	writer.writerow(net['delay']['In'])
	
	#write internal delays delay_Intern
	writer.writerow(['delay_Intern'])
...
def load_RNN(filename):
	"""	Load recurrent neural network object from file
	...
	#read input delays delay_Input
	delay_Input = list(np.array(data[3],dtype=np.int))
	
	#read internal delays delay_Intern
	if data[5] == ['','']:
		delay_Intern = []
	else:
		delay_Intern = list(np.array(data[5],dtype=np.int))
		
	#read output delays dIOut
	if data[7] == ['','']:
		delay_Output = []
	else:
		delay_Output = list(np.array(data[7],dtype=np.int))
	...
	net = construct_RNN(nn,delay_Input,delay_Intern,delay_Output)
	net['normP'] = normP
	net['normY'] = normY
	net['w'] = w
